...
function GeneratorLog() {
  ...
  const handleSave = async (date) => {
    ...
    try {
      ...
      setMessage(`✅ Data saved for ${date}`);
      setError('');
      fetchReadings();
      ...
      setError(`❌ Failed to save data for ${date}`);
      setMessage('');
      setTimeout(() => setError(''), 4000);
    }
  };
  ...
  return (
    <div className="p-4 max-w-5xl mx-auto">
      ...
      {message && (
        <div className="mb-4 p-3 rounded bg-green-100 text-green-800 text-sm shadow">
          {message}
        </div>
      )}
      {error && (
        <div className="mb-4 p-3 rounded bg-red-100 text-red-800 text-sm shadow">
          {error}
        </div>
      )}
